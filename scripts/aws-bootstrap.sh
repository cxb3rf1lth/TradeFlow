...
if [[ -z "$JWT_SECRET" ]]; then
  if command -v openssl >/dev/null 2>&1; then
    ...
    echo "Error: Unable to generate secret. Neither openssl nor /dev/urandom is available." >&2
    echo "Please run the script again and provide a secret manually." >&2
    exit 1
  fi
...
echo "Secrets stored in .env.production and client/.env.production"
